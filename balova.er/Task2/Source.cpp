...
ostream& operator<<(std::ostream& out, Vector& s) {
  ifstream input;
  Vector m1;
  input.open("C:\\Users\\Helen\\source\\repos\\IiP_381908_2_2\\balova.er\\Task2\\in.txt");
  if (!input) {
    ...
    out << endl << s.n << endl;
    cout << "===================данные выведены в консоль======================" << endl;
    input.close();
  }
  return out;
}
...
istream& operator>>(istream& in, Vector& s) {
  ofstream output;
  output.open("C:\\Users\\Helen\\source\\repos\\IiP_381908_2_2\\balova.er\\Task2\\out.txt", ios::trunc);
  if (!output) {
...
Vector& Vector::operator=(Vector& vec) {
  if (vec.n < vec.x * 4) {
    cout << "Тагого вектора не существует";
    exit(0);
  }
  else {
    v[vec.x * 4 - 3] = x1;
